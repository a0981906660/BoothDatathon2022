...
msa_dict <- jsonlite::fromJSON("The NYT Covid Cases Report/msa_dict.json")
...
## Covid cases ----
case <- fread('The NYT Covid Cases Report/msa_cumulative_cases.csv', header = T)[, 1:300]
...
## read pop ----
census_files <- list.files('Urban Region Census/Region_Demographics/',
                           full.names = T)


read_census <- function(file){
  msa_file <- tstrsplit(file, '/')[[3]] %>% str_remove_all("\\.csv")
  dt <- file %>% 
...
case_state[, outbreak := ifelse(cases_per_m > 5000, 1, 0)]
case_state[, week := week(ymd(date))]
rollmean(deaths, k = 3, fill = NA)
